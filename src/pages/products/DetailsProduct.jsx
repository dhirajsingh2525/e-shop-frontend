    ...
    <div className='card-details w-full'>
        <div className='card-show px-[5%] w-full flex justify-between'>
              ...
              <div className='flex flex-col gap-8'>
                 <h1 className='card-details-text text-2xl text-red-300 mt-3'>{product?.title}</h1>
               <p className='w-90 text-red-300'>{product?.description}</p>
               <h2 className='text-red-300 mt-3 text-5xl'>${product?.price}</h2>
       ...
       {user?.isAdmin &&  <form onSubmit={handleSubmit(submitHandler)} className='update-form p-5 w-full'>
            ...
            className='update-form1 w-full text-3xl text-red-300 outline-0 border-b mt-5 p-2'
             />
             <input 
             {...register("title")}
             type="text"
